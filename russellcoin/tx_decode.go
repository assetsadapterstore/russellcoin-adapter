...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := rcTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &rcTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: rcTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := rcTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]rcTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]rcTransaction.SignaturePubkey, 0)
		transHash     = make([]rcTransaction.TxHash, 0)
		addressPrefix rcTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := rcTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := rcTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		txUnlock := rcTransaction.TxUnlock{
	...
	signedTrans, err := rcTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := rcTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]rcTransaction.Vin, 0)
		vouts            = make([]rcTransaction.Vout, 0)
		txUnlocks        = make([]rcTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := rcTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		txUnlock := rcTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: rcTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		out := rcTransaction.Vout{to, uint64(amount.IntPart())}
	...
	/////////构建空交易单
	emptyTrans, err := rcTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := rcTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: rcTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
